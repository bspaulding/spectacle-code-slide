...
class CodeSlide extends React.Component {
  ...
  render() {
    const {code, lang, ranges, color, bgColor, notes, showLineNumbers, headerColor, ...rest} = this.props;
    const {active} = this.state;
    ...
    return (
      <Slide ref='slide' bgColor={slideBg} margin={1} {...rest}>
        {range.title && <CodeSlideTitle color={headerColor}>{range.title}</CodeSlideTitle>}
